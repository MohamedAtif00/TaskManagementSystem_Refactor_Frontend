...
test.describe('Dashboard', () => {
  test('uses sidebar-only navigation without header or footer links', async ({ page }) => {
    ...
    await expect(page.locator('app-footer').getByRole('link')).toHaveCount(0);
    await expect(page.locator('app-sidebar').getByRole('link', { name: 'Dashboard' })).toBeVisible();
  });

  test('hides skeleton after dashboard loads', async ({ page }) => {
    await loginAsOwner(page);
    await expect(page.locator('app-stat-card').first()).toBeVisible();
    await expect(page.locator('app-stat-cards-skeleton')).toHaveCount(0);
  });
